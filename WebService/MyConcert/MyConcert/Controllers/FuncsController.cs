                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < categories.Count; i++)
                            {
                                ...
                                Dictionary<string, object> diccComments = serial.Deserialize<Dictionary<string, object>>(response);
                                dynamic catID = diccComments["id"];                                

                                for (int j = 0; j < bands.Count; j++)
                                {
                                    Dictionary<string, Object> diccB = new Dictionary<string, object>();
                                    diccB.Add("bandID", bands[j].ID.ToString());
                                    diccB.Add("categoryID", catID);
                                    diccB.Add("billboardID", details["ID"]);
                                    string jsonBandToBillboard = Models.UtilityMethods.diccTOstrinJson(diccB);
                                    ...
                                    tweetDicc.Add("tweet", "La banda " + bands[j].name.ToString() + " se ha agregado a la  cartelera " + pJson.name.ToString());
            ...
            try
            {
                ...
                for (int i = 0; i < pJson.categories.Count; i++)
                {
                    ...
                    for (int j = 0; j < category.bands.Count; j++)
                    {
                        ...
                        {
                            string commentsUrl = "http://myconcert1.azurewebsites.net/api/Main/GET/spGetCommentsFromBand/" + band.id.ToString();
                            string json = Models.UtilityMethods.getMethod(commentsUrl);
                            try1 = band.id.ToString();
                    ...
                    for (int i = 0; i < bands.Count; i++)
                    {
                        ...
                        try1 = bands[i].ToString();
                     if (spotifyID.CompareTo("") == 0)
                     {

                         string commentsUrl = "http://myconcert1.azurewebsites.net/api/Main/GET/spGetCommentsFromBand/" + bands[i].id.ToString();
                         string json = Models.UtilityMethods.getMethod(commentsUrl);
                ...
                return try1; //Models.UtilityMethods.diccTOstrinJson(cheffBand);                
        }
        catch (Exception e)
        {
            return e.Message;
